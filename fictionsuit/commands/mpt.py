...
class MPT(CommandGroup):
    ...
    async def cmd_hf_diffuser(self, message: UserMessage, args: str):
        ...
        path = f"/run/media/ponder/ssd0/ml_models/{args}"
    ...
    async def cmd_hf_localize_diffuser(self, message: UserMessage, args: str):
        ...
        path = f"/run/media/ponder/ssd0/ml_models/{path}"
        model = DiffusionPipeline.from_pretrained(model_name, token="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") # DON'T COMMIT THIS SHIT, DUMBASS
